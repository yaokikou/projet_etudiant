...
try {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as actifs FROM Service WHERE actif = 1");
    $services_actifs = $stmt->fetch()['actifs'];
    
    $stmt = $pdo->query("SELECT COUNT(*) as inactifs FROM Service WHERE actif = 0");
    $services_inactifs = $stmt->fetch()['inactifs'];
...
if (isset($_GET['toggle'])) {
    ...
    $_SESSION['success_message'] = "Statut du service modifié.";
    header('Location: services.php');
    exit;
}
...
<body>
        ...
        <div class="main-content">
            ...
            <div class="stats-container">
                ...
                <div class="stat-card actifs">
                    ...
                    <div class="stat-value"><?php echo $services_actifs; ?></div>
                    <div class="stat-label">Services actifs</div>
                </div>
                
                <div class="stat-card inactifs">
                    <i class="fas fa-pause-circle"></i>
                    <div class="stat-value"><?php echo $services_inactifs; ?></div>
            ...
            <div class="admin-content">
                <h2><i class="fas fa-plus"></i> Ajouter un service</h2>
                <form method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="nom_service">Nom du service</label>
                        <input type="text" id="nom_service" name="nom_service" placeholder="Nom du service" required>
                    </div>
            ...
            <!-- Services List -->
            <div class="admin-content">
                <h2><i class="fas fa-list"></i> Liste des services</h2>
                
                <div class="services-grid">
                    <?php foreach ($services as $service): ?>
                        <div class="service-card">
                            <?php if ($service['image'] && file_exists(__DIR__ . '/../assets/img/' . $service['image'])): ?>
                                <img src="../assets/img/<?= htmlspecialchars($service['image']) ?>" alt="Image service" class="service-image">
                            <?php else: ?>
                            ...
                            <div class="service-title"><?= htmlspecialchars($service['nom_service']) ?></div>
                            
                            <div class="service-description">
                                <?= nl2br(htmlspecialchars(substr($service['description'], 0, 150))) ?>
                                <?= strlen($service['description']) > 150 ? '...' : '' ?>
                            </div>
                            
                            <div class="service-status <?= $service['actif'] ? 'status-active' : 'status-inactive' ?>">
                                <?= $service['actif'] ? 'Actif' : 'Inactif' ?>
                            </div>
                            
                            <div class="service-actions">
                                <button class="btn btn-primary" onclick="openEditModal(<?= $service['id'] ?>, '<?= htmlspecialchars($service['nom_service']) ?>', '<?= htmlspecialchars($service['description']) ?>', '<?= htmlspecialchars($service['image']) ?>')">
                                    <i class="fas fa-edit"></i> Modifier
                                </button>
                                <a href="?toggle=<?= $service['id'] ?>" class="btn btn-warning">
                                    <i class="fas fa-toggle-on"></i> <?= $service['actif'] ? 'Désactiver' : 'Activer' ?>
                                </a>
                                <a href="?delete=<?= $service['id'] ?>" class="btn btn-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce service ?')">
                ...
                <?php if (empty($services)): ?>
                    <div style="text-align: center; padding: 40px; color: #777;">
                        <i class="fas fa-cogs" style="font-size: 48px; margin-bottom: 15px;"></i>
    ...
    <script>
        ...
        function openEditModal(id, nom, description, image) {
